...
const UserStat = ({ props }: Props) => {
  return (
    <div className="flex flex-col gap-2 py-3">
      <div className="flex justify-between items-center p-3 bg-[#3d6076] rounded-lg">
        <p className="text-base text-[#adc8d1] font-bold">전투력</p>
        <div className="text-[#F2F3D1] font-bold text-2xl">
          {geKoreanNumber(Number(props.final_stat[42].stat_value))}
        </div>
        <div className="w-[40px]"></div>
      </div>
      <div className="flex flex-wrap bg-[#86929E] rounded-lg p-2">
        {topUserStat(props).map((item: FinalStat, idx: number) => (
          <div
            className="flex justify-between w-1/2 py-1 px-2 text-14 text-white mobile:w-full"
            key={idx}
          >
            <p>{item.stat_name}</p>
            <p>{item.stat_value}</p>
      ...
      <div className="flex flex-wrap bg-[#6b7785] rounded-lg p-2">
